...
| 15 | **A captcha marker marks a page blocked however much content it has.** `detect_blocked_kind` exempts `captcha` (and `bot_challenge`) from the low-visible-text guard, so any page that merely loads reCAPTCHA for a form is treated as an interstitial. Measured on scaler.com: `/events` is a real events page with 9,058 characters of visible text, matched only through script and class names such as `trigger-recaptcha` (163 occurrences), and was reported `blocked: captcha`, which set the whole report to `partial: true, partial_reason: "blocked"`. No test covers a captcha marker. None of the 28 saved real homepages carries one. On a homepage the same rule would block the homepage-anchored analyzers; that is inferred from the blocked-homepage behaviour, not measured. | Medium | **Open, deliberately not changed (decided 2026-09-13).** Recorded as a known limitation in `sample-report/README.md`. |
| 16 | **A redirect between the bare domain and `www` can select the homepage twice.** `build_artifact` passes the requested `url` to `select_pages` (`_artifact.py:262`), not `final_url`, and deduplication compares exact URLs while `same_site` ignores `www.`. Measured on `https://scaler.com` (301 to `https://www.scaler.com/`): `pages_checked` holds both addresses, the homepage takes two of five slots, and the other selected pages resolve against the pre-redirect host and redirect again. None of the five earlier sample reports shows it. | Low | **Open, deliberately not changed (decided 2026-09-13).** Recorded as a known limitation in `sample-report/README.md`. |
| 17 | **`extraction.facts_not_image_only` treats `alt=""` as a missing text equivalent.** `has_text_equivalent = len(alt) >= 10`, so an image explicitly marked decorative is suspect whenever its filename looks data-like, although the function's docstring says decorative imagery is ignored. Measured on scaler.com: 19 of 77 images flagged, the named ones 42×42 icons such as `rectangle-737.webp` with `alt=""`. | Low | **Open, deliberately not changed (decided 2026-09-13).** Treated as the documented statistical-signal limitation; described in the scaler.com paragraph of `sample-report/README.md`. |
...
- 2026-09-13 — **scaler.com added as the sixth sample report; three defects found and deliberately
  left open (issues 15, 16, 17, user decision).** Live run of `--url https://scaler.com` after
  checking its robots.txt (homepage allowed, no prose restriction): 94, coverage 1.0, 3 medium
  findings, Tier A, 60 s, empty stderr. Saved unedited as `sample-report/scaler.com.json` and
  `scaler.com.stderr.txt`.
  **Checked against the live page before saving:** `og:type` genuinely missing; the raw homepage has
  no `ul`, `ol`, `table` or `dl`; identity is an `EducationalOrganization` with logo, contactPoint
  and four sameAs. The image finding is a false signal (issue 17). The report's
  `partial_reason: "blocked"` comes only from `/events`, a real content page flagged through
  reCAPTCHA form scripts (issue 15). The homepage was selected twice, as `scaler.com/` and
  `www.scaler.com/`, because selection uses the requested rather than the redirected URL (issue 16).
