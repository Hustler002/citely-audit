"""Phase 9 — the labelled corpus, as a CI gate.
...
Also here: three properties that were described in the Phase-1 scaffold stubs and never actually
asserted anywhere. Those stub files were deleted in this phase — their skip reason still read
"checks not implemented yet", which stopped being true in Phase 4 — so the intent lands here
instead of being lost with them.
"""
...
def test_no_page_is_blamed_for_something_it_does_right(outcome):
    """Precision. The rubric penalises false positives as hard as misses, and they are the failure
    mode a reader notices first: being told to fix something that is already correct."""
...
def test_every_archetype_scores_in_its_expected_band(outcome):
    """PLAN §8 asks for sensible behaviour per archetype, asserted as ranges rather than exact
...
def test_a_javascript_shell_produces_one_root_cause_not_a_cascade(outcome):
    """PLAN §6.3: dependency suppression turns ~12 downstream false positives into one finding
...
def test_the_checked_in_sample_report_still_matches_a_fresh_run():
    """The sample is regenerated by hand every phase, which is exactly the step that goes stale.
...
def test_the_nine_archetypes_are_represented():
    """PLAN §8 names nine archetypes. Consent-wall lives in the fixture server rather than here,
